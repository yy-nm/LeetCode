...
class Solution {
    ...
    void generate(const vector<int>& candidates, int index, int target, vector<int>& middle, vector<vector<int>>& result) {
        ...
        if (index < candidates.size() - 1) {
            if (middle.size() == 0) {
                for (int i = 1; i <= count; i++) {
                    ...
                    answer.insert(answer.end(), middle.begin(), middle.end());
                    generate(candidates, index + 1, target - i * v, answer, result);
                }
            }
            else {
...
public:
    vector<vector<int>> combinationSum(vector<int>& candidates, int target) {
        ...
        for (int i = 0; i < candidates.size(); i++) {
            generate(candidates, i, target, v, result);
        }
